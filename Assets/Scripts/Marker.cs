...
public class Marker : MonoBehaviour {
	...
	private void Update() {
		...
		if(oldMarkerPosition != Vector3.zero && oldMarkerRotation != Quaternion.identity) {
			transform.position = Vector3.Lerp(transform.position, newMarkerPosition, (oldMarkerPosition - newMarkerPosition).magnitude / (WebCamera.instance.ArucoSquareDim * positionLerpMax));
